...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  const [mapState, dispatchMap] = useReducer(
    (
      ...
      action:
        | { type: "init"; instance: MapLibreGL.Map }
        | { type: "loaded" }
        | { type: "styleLoaded" }
        | { type: "styleChanging" }
        | { type: "reset" },
      ...
      switch (action.type) {
        ...
        case "styleLoaded":
          return { ...state, styleLoaded: true };
        case "styleChanging":
          return { ...state, styleLoaded: false };
        case "reset":
          return { instance: null, loaded: false, styleLoaded: false };
      }
    },
    { instance: null, loaded: false, styleLoaded: false },
  );
  const mapInstance = mapState.instance;
  const isLoaded = mapState.loaded;
  const isStyleLoaded = mapState.styleLoaded;
  ...
  useEffect(() => {
    ...
    const styleDataHandler = () => {
      ...
      styleTimeoutRef.current = setTimeout(() => {
        dispatchMap({ type: "styleLoaded" });
        if (projection) {
          map.setProjection(projection);
        }
      }, 100);
    };
    const loadHandler = () => dispatchMap({ type: "loaded" });
    ...
    dispatchMap({ type: "init", instance: map });
  ...
  useEffect(() => {
    ...
    dispatchMap({ type: "styleChanging" });
...
export { Map, MapMarker, MarkerContent, MarkerPopup };
